    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene){
        ...
        for(GLuint i = 0; i < mesh->mNumVertices; i++){
            ...
            if(mesh->mTextureCoords[0]){
                ...
                // In realta' , e' possibile avere fino a 8 diverse coordinate texture: per altri modelli e formati, bisognerÃ² adattare questo codice.
...
/*
 * Metodo che effettua il caricamento della texture da un file su disco (se non è stato fatto precedentemente) e crea la Texture Unit di OpenGL
 * Prende in input i seguenti valori:
 * - path: char*, variabile in cui è contenuto il path della texture
 * - directory: string, variabile in cui è contenuta la directory della texture
